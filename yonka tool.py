...
class Render:
    ...
    def render_ascii(self):
        clear()
        title(f"yonka tool| Connected as {os.getlogin()} | made by Tips-Discord")
        edges = ["╗", "║", "╚", "╝", "═", "╔"]
        ascii = f"""
██╗   ██╗ ██████╗ ███╗   ██╗██╗  ██╗ █████╗     ████████╗ ██████╗  ██████╗ ██╗     
╚██╗ ██╔╝██╔═══██╗████╗  ██║██║ ██╔╝██╔══██╗    ╚══██╔══╝██╔═══██╗██╔═══██╗██║     
...
class Raider:
    ...
    def get_discord_cookies(self):
        try:
            ...
            match response.status_code:
                ...
                case _:
                    console.log("ERROR", C["blue"], "Failed to get cookies using Static")
            ...
            console.log("ERROR", C["blue"], "get_discord_cookies", e)
    ...
    def super_properties(self):
        try:
            ...
            console.log("ERROR", C["blue"], "get_super_properties", e)
    ...
    def joiner(self, token, invite):
        try:
            ...
            match response.status_code:
                ...
                case _:
                    console.log("FAILED", C["blue"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**", response.json().get("message"))
        except Exception as e:
    ...
    def leaver(self, token, guild):
        try:
            ...
            console.log("FAILED", C["blue"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**", e)
...
class Menu:
    ...
    def main_menu(self):
        ...
        if check(base64.b64decode("eW9ua2E=").decode("utf-8")) and check(base64.b64decode("bWFkZSBieSBUaXBzLURpc2NvcmQ=").decode("utf-8")):
            pass
        else:
    ...
    def nick_changer(self):
        title(f"Yonka - Nickname Changer")
    ...
    def checker(self):
        title(f"Yonka - Checker")
        self.raider.token_checker()

    @wrapper
    def reactor(self):
        title(f"Yonka - Reactor")
    ...
    def formatter(self):
        title(f"Yonka - Formatter")
        self.run(self.raider.format_tokens, [()])
    
    @wrapper
    def button(self):
        title(f"Yonka - Clicker")
        Link = input(console.prompt("Message Link"))
        if Link == "" or not Link.startswith("https://"):
            self.main_menu()
    ...
    @wrapper
    def onboard(self):
        title(f"yonka - Onboarding Bypass")
        guild_id = input(console.prompt("Guild ID"))
        if guild_id == "":
            self.main_menu()
